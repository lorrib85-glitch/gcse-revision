...
export default function FaceTheExaminerMain(props) {
  ...
  const backerImage = examiner.backgroundImage || IMAGES[module.subject] || IMAGES.History
  ...
  const bottomPanelHeight = revealPanelVisible && isReveal ? 292 : 0
  ...
  return (
    <>
      <style>{`
        ...
        .fte-chip { display: inline-flex; align-items: center; justify-content: center; min-height: 44px; padding: 9px 14px; border-radius: 999px; font-family: ${TYPE.body.fontFamily}; font-size: ${TYPE.bodySmall.fontSize}; font-weight: 600; cursor: pointer; border: 1px solid ${GENERAL.line.medium}; background: ${GENERAL.surfaceTint}; color: ${GENERAL.cinematic.textMuted}; transition: background 160ms ease, border-color 160ms ease, color 160ms ease, transform 160ms ease; user-select: none; }
        ...
        .fte-improve-btn { display: inline-flex; align-items: center; min-height: 32px; padding: 4px 9px; border-radius: 10px; margin-left: 5px; font-family: ${TYPE.body.fontFamily}; font-size: 11px; font-weight: 700; letter-spacing: 0.02em; cursor: pointer; vertical-align: middle; background: ${accent}18; border: 1px solid ${accent}55; color: ${accent}; transition: background 160ms ease, border-color 160ms ease, color 160ms ease; }
        ...
        .fte-textarea { width: 100%; box-sizing: border-box; background: ${GENERAL.surfaceTint}; border: 1px solid ${GENERAL.line.strong}; border-radius: 10px; padding: 10px 12px; color: ${GENERAL.cinematic.textFact}; font-family: ${TYPE.body.fontFamily}; font-size: ${TYPE.bodySmall.fontSize}; line-height: 1.6; resize: vertical; outline: none; margin-top: 8px; }
      ...
      <div style={{ position: 'fixed', inset: 0, zIndex: 900, isolation: 'isolate', background: bg, display: 'flex', flexDirection: 'column', overflow: 'hidden' }}>
        {backerImage && (
          <div style={{
            ...
            filter: 'grayscale(4%) brightness(0.7)',
          }} />
        )}
        ...
        <div style={{
          ...
          <div style={{ ...TYPE.metadata, color: GENERAL.cinematic.textMuted, textAlign: 'center', minWidth: 0 }}>
            Face the examiner
            <span style={{ color: accent, marginLeft: 6 }}>· {module.subject || 'History'}</span>
          </div>
        ...
        {showTabs && (
          <div style={{
            ...
            <SegmentedControl
              ...
              options={[
                { value: 'answer', label: examiner.answerTabLabel || 'Student answer' },
        ...
        <div
          ...
          style={{
            ...
            paddingBottom: bottomPanelHeight > 0 ? bottomPanelHeight + 20 : 108,
        ...
        >
          ...
          {remarkError && (
            <div style={{ marginTop: 12, padding: '10px 14px', borderRadius: 10, background: `${GENERAL.feedbackIncorrect}18`, border: `1px solid ${GENERAL.feedbackIncorrect}55`, ...TYPE.bodySmall, color: GENERAL.feedbackIncorrect }}>
              {remarkError}
            </div>
          )}
        </div>
        ...
        {!isReveal && (
          <div style={{
            ...
            background: 'linear-gradient(180deg, rgba(8,9,13,0) 0%, rgba(8,9,13,0.92) 28%, rgba(8,9,13,1) 100%)',
            position: 'relative',
            zIndex: 2,
            opacity: introBlocksMainInteraction ? 0 : 1,
              ...
              <>
                {!readyToReveal && (
                  <div style={{ ...TYPE.bodySmall, color: GENERAL.cinematic.textMuted, textAlign: 'center', marginBottom: 8 }}>
                    {guessedMark === null
                      ? 'Choose a mark to continue.'
                      : 'Choose at least one observation to continue.'}
                  </div>
                )}
                <ContinueCTA
                  accent={accent}
            ...
            {isRemarking && (
              <div style={{ height: 56, display: 'flex', alignItems: 'center', justifyContent: 'center', ...TYPE.bodySmall, color: GENERAL.cinematic.textMuted }}>
        ...
        {questionIntroVisible && phase === 'reading' && (
          <div style={{
            ...
            <div style={{
              ...
              <div style={{ ...TYPE.eyebrow, color: accent, textTransform: 'none', marginBottom: SCREEN_TEXT_LAYOUT.blockGap }}>
            ...
            <div style={{
              ...
              <ContinueCTA accent={accent} label="Show the model answer" onClick={startQuestionIntroDock} />
            </div>
          </div>
        )}
        ...
        {isReveal && revealPanelVisible && (
          <div style={{
            ...
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 12, marginBottom: 12 }}>
              <div>
                <div style={{ ...TYPE.metadata, color: GENERAL.cinematic.textMuted, marginBottom: 2 }}>Examiner mark</div>
                <div style={{ ...TYPE.displayHero, color: accent }}>{examiner.mark}/{examiner.marks}</div>
              </div>
              <div>
                <div style={{ ...TYPE.metadata, color: GENERAL.cinematic.textMuted, marginBottom: 2 }}>Your mark</div>
                <div style={{ ...TYPE.displaySection, color: GENERAL.cinematic.textPrimary, paddingTop: 7 }}>{guessedMark ?? '—'}/{examiner.marks}</div>
              </div>
            </div>
            <p style={{ ...TYPE.bodySmall, color: GENERAL.cinematic.textSecondary, margin: '0 0 6px' }}>{examiner.summary}</p>
            {guessedMark !== null && (
              <p style={{ ...TYPE.bodySmall, color: GENERAL.cinematic.textMuted, margin: '0 0 16px' }}>
                {markComparisonText(guessedMark, examiner.mark)}
              </p>
            )}
            {canImprove ? (
              <ContinueCTA
                accent={accent}
                label={examiner.improveCtaLabel || 'Fix the weakest sentence'}
                onClick={() => {
