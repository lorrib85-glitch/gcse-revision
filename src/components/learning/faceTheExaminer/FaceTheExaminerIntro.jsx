    ...
    <>
      ...
      <div style={{ position: 'fixed', inset: 0, zIndex: 1000, background: '#080C10', overflow: 'hidden' }}>
        ...
        <div style={{ position: 'absolute', left: 28, right: 28, bottom: 80 }}>
          {titleVisible && <div style={{ ...TYPE.impactTitle, fontSize: 48, lineHeight: 0.95, color: accent, marginBottom: 20, animation: 'fte-up 700ms ease both' }}>Face the<br />examiner</div>}
          {sub1Visible && <p style={{ ...TYPE.overlayBody, color: 'rgba(245,238,225,0.88)', margin: '0 0 4px', animation: 'fte-up 600ms ease both' }}>This answer sounds convincing.</p>}
          {sub2Visible && <p style={{ ...TYPE.overlayBody, color: 'rgba(245,238,225,0.88)', margin: 0, animation: 'fte-up 600ms ease both' }}>But would it get top marks?</p>}
        </div>
      </div>
    </>
  )
