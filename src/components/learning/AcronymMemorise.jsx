...
import { ScreenIntro } from '../core/ScreenText.jsx'
...
export default function AcronymMemorise({ block, subject }) {
  ...
  const fallbackTarget = items.length > 0
    ? `these ${items.length} key ideas`
    : 'the key ideas'
  const memoryTarget = block.memoryTarget || block.rememberTarget || block.topic || fallbackTarget
  const introduction = sentence(
    block.intro || `${acronym} is an easy way to remember ${memoryTarget}`,
  )
  const instruction = isTestMode
    ? (block.testInstruction || 'Say each meaning before you tap to check.')
    : (block.instruction || 'Tap each letter to reveal what it stands for.')
  const openingCopy = `${introduction} ${instruction}`
