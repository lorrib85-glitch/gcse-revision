    ...
    <CinematicShell style={{ background: pageBg, zIndex: 1000 }}>
      <style>{`
        ...
        @keyframes ihi-pulse {
          0%,100% {
            box-shadow:
              0 0 0 2px rgba(${accentRgb},0.55),
              0 0 22px rgba(${accentRgb},0.72),
              0 0 52px rgba(${accentRgb},0.36);
          }
          50% {
            box-shadow:
              0 0 0 3px rgba(${accentRgb},0.75),
              0 0 36px rgba(${accentRgb},0.92),
              0 0 72px rgba(${accentRgb},0.52);
          }
        }
      ...
      <div style={{
        ...
        <div style={{ position: 'absolute', left: '28px', right: '28px', bottom: 'max(72px, calc(env(safe-area-inset-bottom, 0px) + 72px))', zIndex: 5 }}>
          <div style={{ ...TYPE.displayHero, color: imageText }}>
            {titleLines.map((line, i) => <div key={i} style={{ animation: `ihi-line-in 400ms cubic-bezier(0.22,1,0.36,1) ${200 + i * 420}ms both` }}>{line}</div>)}
          </div>
      ...
      <div style={{ position: 'absolute', inset: 0, overflowY: 'auto', opacity: isExplore ? 1 : 0, transition: 'opacity 500ms cubic-bezier(0.22,1,0.36,1)', pointerEvents: isExplore ? 'auto' : 'none' }}>
        <div style={{ position: 'relative', width: '100%', marginTop: 'calc(env(safe-area-inset-top, 0px) + 80px)' }}>
          ...
          <div style={{ position: 'absolute', top: '16px', right: '14px', pointerEvents: 'none', zIndex: 6 }}>
            <SequenceProgress total={hotspots.length} current={-1} viewed={hotspots.map((h, i) => visited.has(h.id) ? i : -1).filter(i => i >= 0)} accent={accent} accentRgb={accentRgb} compact={true} ariaLabel="Hotspot progress" />
          </div>
          ...
          {hotspots.map(h => {
            ...
            return (
              <div key={h.id}>
                <button type="button" className="ihi-dot" aria-label={`Explore ${h.title}`} aria-pressed={isSelected} onClick={() => handleTap(h.id)} style={{ position: 'absolute', left: `${h.x}%`, top: `${h.y}%`, transform: 'translate(-50%,-50%)', width: '28px', height: '28px', borderRadius: '50%', border: 'none', padding: 0, cursor: 'pointer', background: isVisited ? `rgba(${accentRgb},0.16)` : 'rgba(255,255,255,0.04)', boxShadow: isSelected ? `0 0 0 2px rgba(${accentRgb},0.75), 0 0 22px rgba(${accentRgb},0.92), 0 0 56px rgba(${accentRgb},0.45)` : isVisited ? `0 0 0 1px rgba(${accentRgb},0.38), 0 0 14px rgba(${accentRgb},0.52), 0 0 36px rgba(${accentRgb},0.22)` : `0 0 0 2px rgba(${accentRgb},0.55), 0 0 22px rgba(${accentRgb},0.72), 0 0 52px rgba(${accentRgb},0.36)`, animation: !isVisited ? 'ihi-pulse 2.8s ease-in-out infinite' : 'none', transition: 'box-shadow 280ms cubic-bezier(0.22,1,0.36,1)', display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: isSelected ? 8 : 6 }}>
                  <div style={{ width: isSelected ? '10px' : isVisited ? '8px' : '8px', height: isSelected ? '10px' : isVisited ? '8px' : '8px', borderRadius: '50%', background: isVisited ? accent : 'rgba(255,255,255,0.85)', boxShadow: isVisited ? `0 0 8px ${accent}` : 'none', transition: 'all 280ms cubic-bezier(0.22,1,0.36,1)' }} />
                </button>
      ...
      <div style={{ position: 'absolute', bottom: 0, left: 0, right: 0, padding: '0 22px max(18px, env(safe-area-inset-bottom, 0px))', transform: (selected && isExplore) ? 'translateY(0)' : 'translateY(calc(100% + 28px))', transition: 'transform 380ms cubic-bezier(0.22,1,0.36,1)', zIndex: 30, pointerEvents: selected ? 'auto' : 'none' }}>
        {selected && <article style={{ background: cardBg, color: cardText, border: `1px solid ${cardRule}`, borderRadius: RADII.panel, padding: '22px 22px 20px', boxShadow: `0 22px 70px rgba(0,0,0,0.44), 0 0 0 1px rgba(${accentRgb},0.08)`, maxHeight: '47vh', overflowY: 'auto' }}>
          ...
          {selectedRows.map((row, index) => <section key={row.label} style={{ display: 'grid', gridTemplateColumns: '10px 1fr', columnGap: '14px', padding: '16px 0', borderBottom: index === selectedRows.length - 1 ? 'none' : `1px solid ${cardRule}` }}>
            ...
            <div>
              <h3 style={{ ...TYPE.label, margin: '0 0 5px', color: cardText }}>{row.label}</h3>
              <p style={{ ...TYPE.bodySmall, margin: 0, color: cardMuted }}>{row.body}</p>
            </div>
          </section>)}
        </article>}
      </div>
