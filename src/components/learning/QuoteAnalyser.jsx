...
function QuoteWord({ word, index, visibleWords, accent, accentRgb, activeWord, onSelect }) {
  ...
  if (shouldHighlight) {
    return (
      <button type="button" onClick={() => onSelect(stripped)} aria-label={`Analyse the word ${stripped}`} style={{ display: 'inline-block', opacity: isVisible ? 1 : 0, color: accent, background: isActive ? `rgba(${accentRgb}, 0.24)` : `rgba(${accentRgb}, 0.12)`, border: `1px solid rgba(${accentRgb}, ${isActive ? 0.54 : 0.30})`, borderBottom: `3px solid rgba(${accentRgb}, ${isActive ? 0.94 : 0.70})`, borderRadius: 10, boxShadow: isActive ? `0 0 10px rgba(${accentRgb}, 0.38), 0 0 28px rgba(${accentRgb}, 0.30), inset 0 0 10px rgba(${accentRgb}, 0.10)` : `0 0 10px rgba(${accentRgb}, 0.22), 0 0 24px rgba(${accentRgb}, 0.18), inset 0 0 8px rgba(${accentRgb}, 0.07)`, padding: '0 0.12em 0.03em', margin: '0 0.06em 0.12em 0', font: 'inherit', lineHeight: 'inherit', letterSpacing: 'inherit', textShadow: `0 0 10px rgba(${accentRgb}, 0.55), 0 0 24px rgba(${accentRgb}, 0.34)`, cursor: 'pointer', transition: 'opacity 0.28s ease, background 0.18s ease, border-color 0.18s ease, box-shadow 0.18s ease, transform 0.18s ease', transform: isActive ? 'translateY(-1px)' : 'none', WebkitTapHighlightColor: 'transparent' }}>
        {word}{' '}
      </button>
    )
  }
