...
function resolveContext(block, literaryMeta) {
  ...
  return {
    label: String(supplied.label || '').trim(),
    beats: [literaryMeta.scene ? `${speaker} speaks in ${literaryMeta.scene}.` : 'This quote comes from a key moment in the text.'],
...
function EditorialSection({ children, accentRgb }) {
  return (
    <section style={{ position: 'relative', padding: '26px 10px 30px' }}>
      <div aria-hidden="true" style={{ position: 'absolute', inset: '0 0 auto', height: 1, background: `linear-gradient(90deg, transparent, rgba(${accentRgb}, 0.42), transparent)` }} />
      {children}
    </section>
  )
}
          ...
          return (
            <span
              ...
              style={{
                ...
                color: 'inherit',
                boxShadow: variant === 'analysis' && marked ? `inset 0 -0.18em rgba(${accentRgb}, 0.22)` : 'none',
              }}
            >
              {word}{' '}
            </span>
...
function StarterPrompts({ starters, heading, expanded, onToggle, onChoose, text }) {
  ...
  return (
    <div style={{ marginTop: 16, borderTop: `1px solid ${GENERAL.line.soft}`, borderBottom: `1px solid ${GENERAL.line.soft}` }}>
      <button type="button" aria-expanded={expanded} aria-controls={regionId} onClick={onToggle} style={{ width: '100%', minHeight: 52, display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: 16, padding: '13px 0', border: 0, background: 'none', color: text.textPrimary, textAlign: 'left', cursor: 'pointer', ...TYPE.bodyStrong }}>
        <span>{heading}</span>
      ...
      {expanded && (
        <div id={regionId} className="qa-motion" style={{ paddingBottom: 6, borderTop: `1px solid ${GENERAL.line.soft}`, animation: 'qa-rise 0.24s ease both' }}>
          {starters.map(starter => (
            <button key={starter} type="button" onClick={() => onChoose(starter)} style={{ width: '100%', minHeight: 46, padding: '11px 0', border: 0, borderBottom: `1px solid ${GENERAL.line.soft}`, background: 'none', color: text.textFact, textAlign: 'left', cursor: 'pointer', ...TYPE.bodySmall }}>
              {starter}
            </button>
          ))}
        </div>
    ...
    <div role="dialog" aria-modal="true" aria-label={`Word focus: ${word}`} style={{ position: 'fixed', inset: 0, zIndex: 180 }}>
      ...
      <div className="qa-motion" style={{ position: 'absolute', left: 12, right: 12, bottom: 'calc(12px + env(safe-area-inset-bottom))', maxWidth: 420, margin: '0 auto', padding: 18, borderRadius: 26, background: 'linear-gradient(180deg, rgba(59,38,38,0.98), rgba(31,28,27,0.99))', border: `1px solid rgba(${accentRgb}, 0.28)`, boxShadow: GENERAL.shadow.overlay, animation: 'qa-rise 0.24s ease both' }}>
        <div style={{ display: 'flex', justifyContent: 'space-between', gap: 12, marginBottom: 14 }}>
          <div>
            <div style={{ ...TYPE.label, color: GENERAL.cinematic.textSecondary, marginBottom: 5 }}>{content.technique}</div>
            <div style={{ fontFamily: "'IBM Plex Serif', Georgia, serif", fontSize: 29, color: parchment }}>“{word}”</div>
          </div>
        ...
        {content.sentence && (
          <div style={{ padding: 12, borderRadius: 16, background: `rgba(${accentRgb}, 0.10)`, border: `1px solid rgba(${accentRgb}, 0.18)` }}>
            <div style={{ ...TYPE.label, color: GENERAL.cinematic.textSecondary, marginBottom: 5 }}>Use it</div>
            <p style={{ ...TYPE.body, color: parchment, margin: 0 }}>{content.sentence}</p>
          </div>
        )}
      </div>
    </div>
...
export default function QuoteAnalyser({ block, subject = 'English', onContinue }) {
  ...
  const wordAnalysisNextStep = block.wordAnalysisNextStep === 'essay' ? 'essay' : 'meaning'
  const wordAnalysisCompleteLabel = block.wordAnalysisCompleteLabel || (wordAnalysisNextStep === 'essay' ? 'Use it in an essay' : 'Next: build the meaning')
  const markedWordKeys = [...new Set(words.map(clean).filter(key => analysisWords[key]))]
  ...
  useEffect(() => {
    ...
    return () => {
      window.clearTimeout(attribution)
  ...
  const interpretationLayout = { padding: '4px 4px 24px', animation: 'qa-rise 0.42s ease 0.08s both' }
        ...
        {backgroundImage && (
          <img
            ...
            style={{
              position: 'absolute',
              inset: cinematic ? '-3%' : 0,
              width: cinematic ? '106%' : '100%',
              height: cinematic ? '106%' : '100%',
              objectFit: 'cover',
              objectPosition: block.backgroundPosition || 'center',
              filter: cinematic ? 'brightness(0.44) saturate(0.78) contrast(1.08)' : 'brightness(0.30) saturate(0.72)',
              opacity: cinematic ? 0.58 : 0.34,
              animation: cinematic ? 'qa-drift 18s ease-in-out infinite' : 'none',
            }}
          />
        )}
  ...
  function Nav({ label }) {
    return (
      <div style={{ display: 'flex', alignItems: 'center', minHeight: 36, margin: '0 6px 4px' }}>
        <button type="button" onClick={back} style={{ minHeight: 36, border: 0, background: 'none', color: text.textMuted, cursor: 'pointer', padding: 0, ...TYPE.caption, fontSize: 13 }}>← Back</button>
        <div style={{ flex: 1, textAlign: 'right', ...TYPE.caption, fontSize: 13, color: text.textMuted }}>{label}</div>
      </div>
    )
  }
        ...
        <main style={{ ...page, paddingLeft: 22, paddingRight: 22 }}>
          ...
          <div style={{ padding: 'clamp(44px, 6.5vh, 70px) 4px 32px' }}>
            ...
            <div style={{ display: 'grid', gap: 14 }}>
              {context.beats.map((beat, index) => <p key={`${beat}-${index}`} className="qa-motion" style={{ ...TYPE.displaySection, fontWeight: 480, color: text.textPrimary, margin: 0, animation: `qa-rise 0.55s ease ${index * 0.32}s both` }}>{beat}</p>)}
            </div>
  ...
  if (step === 'interpret') {
    const needsRetry = Boolean(feedback) && (feedback.responseQuality === 'non_answer' || (!feedback.responseQuality && feedback.understanding === 'starting' && !feedback.strengths?.length))
          ...
          <section className="qa-motion" style={interpretationLayout}>
            {!feedback ? (
              <>
                ...
                <textarea ref={textareaRef} className="qa-interpretation-input" value={interpretation} onChange={event => { setInterpretation(event.target.value); setCheckError('') }} placeholder={block.interpretationPlaceholder || 'Write your interpretation...'} maxLength={1400} rows={5} aria-label="Your interpretation of the quote" style={{ '--qa-focus-accent': accent, '--qa-focus-ring': `rgba(${accentRgb}, 0.18)`, '--qa-placeholder': text.textMuted, width: '100%', height: 138, minHeight: 138, resize: 'vertical', boxSizing: 'border-box', padding: '15px 16px', borderRadius: 16, border: `1px solid ${GENERAL.line.strong}`, outline: 'none', background: GENERAL.surfaceTint, color: parchment, caretColor: parchment, transition: 'border-color 0.18s ease, box-shadow 0.18s ease', ...TYPE.bodyStrong, fontWeight: 400, lineHeight: 1.55 }} />
                <StarterPrompts starters={interpretationStarters} heading={interpretationStarterHeading} expanded={promptsExpanded} onToggle={() => setPromptsExpanded(current => !current)} onChoose={insertStarter} text={text} />
              ...
              <>
                ...
                <div style={{ borderTop: `1px solid ${GENERAL.line.soft}` }}>
                  {support.sentenceStarters.map(starter => <button key={starter} type="button" onClick={() => retryInterpretation(`${starter} `)} style={{ minHeight: 48, width: '100%', padding: '11px 2px', border: 0, borderBottom: `1px solid ${GENERAL.line.soft}`, background: 'none', color: text.textPrimary, textAlign: 'left', cursor: 'pointer', ...TYPE.bodySmall }}>{starter}</button>)}
                </div>
                {showInterpretationHint && <div aria-live="polite" style={{ marginTop: 14, paddingLeft: 13, borderLeft: `2px solid rgba(${accentRgb}, 0.62)` }}><div style={{ ...TYPE.label, color: text.textSecondary, marginBottom: 5 }}>One hint</div><p style={{ ...TYPE.bodySmall, color: text.textPrimary, margin: 0 }}>{support.hint}</p></div>}
              ...
              <>
                ...
                <div style={{ paddingTop: 16, borderTop: `1px solid ${GENERAL.line.soft}` }}>
                  ...
                  {feedback.strengths?.length ? <div style={{ display: 'grid', gap: 18 }}>{feedback.strengths.map((insight, index) => <FeedbackInsight key={`${insight.idea}-${index}`} insight={insight} accentRgb={accentRgb} />)}</div> : <p style={{ ...TYPE.body, color: text.textSecondary, margin: 0 }}>You have made a start. Now connect the idea to one exact word from the quote.</p>}
                </div>
  ...
  if (step === 'meaning') {
    return (
      <div style={outer}>
        <Atmosphere cinematic />
        <main style={{ ...page, overflowY: 'auto' }}>
          <Nav label={block.meaningLabel || 'Build the meaning'} />
          <EditorialSection accentRgb={accentRgb}>
  ...
  if (step === 'essay') {
    return (
      <div style={outer}>
        <Atmosphere cinematic />
        <main style={{ ...page, overflowY: 'auto' }}>
          <Nav label={block.essayNavLabel || 'Use it in an essay'} />
          <EditorialSection accentRgb={accentRgb}>
