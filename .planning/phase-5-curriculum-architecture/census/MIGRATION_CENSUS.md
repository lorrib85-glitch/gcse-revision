...
| `soc_family` | `module` | `sociology-aqa-families (+ sociology-aqa-key-concepts)` | Splits. Title says "Sociology of the Family" but only soc4 and soc6 are Families; soc1–soc3 are key concepts and sociological approaches that run across both papers. See open decision OD-4. |
...
| `soc1` | `chapter` | `sociology-aqa-key-concepts` | Key-concepts / approaches chapter, not Families content, despite sitting in the `soc_family` module and the `sociology/families/` content directory. Built content. See open decision OD-4. |
| `soc2` | `chapter` | `sociology-aqa-key-concepts` | Key-concepts / approaches chapter, not Families content, despite sitting in the `soc_family` module and the `sociology/families/` content directory. Built content. See open decision OD-4. |
| `soc3` | `chapter` | `sociology-aqa-key-concepts` | Key-concepts / approaches chapter, not Families content, despite sitting in the `soc_family` module and the `sociology/families/` content directory. Built content. See open decision OD-4. |
