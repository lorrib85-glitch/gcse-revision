...
- `src/components/learning/TimelineChain.jsx` fully migrated: the 1 clean `CinematicShell` root `background: '#08090D'` site now uses `GENERAL.backgroundApp`. No other exact-hex target sites were present in this file (a `background: 'rgba(8,9,13,0.55)'` step-number badge overlay is decorative and was left untouched). `import { GENERAL } from '../../constants/generalTheme.js'` added.
- `src/components/learning/VisualNarrativeScreen.jsx` fully migrated: the 1 clean `CinematicShell` `background: '#08090D'` site now uses `GENERAL.backgroundApp`. No other exact-hex target sites were present in this file. `import { GENERAL } from '../../constants/generalTheme.js'` added.
- `src/components/learning/ExaminerExplainsScreen.jsx` fully migrated: the 1 clean `CinematicShell` `background: '#08090D'` site now uses `GENERAL.backgroundApp`. No other exact-hex target sites were present in this file. `import { GENERAL } from '../../constants/generalTheme.js'` added.
- `src/components/learning/VisualLearning.jsx` fully migrated: 1 clean `CinematicShell` `background: '#08090D'` site now uses `GENERAL.backgroundApp`. No remaining exact B9 target hexes in the file — the `rgba(8,9,13,...)` gradient overlays are decorative, non-exact matches and were left untouched. `import { GENERAL } from '../../constants/generalTheme.js'` added.
...
### Remaining work
- Migrate one area at a time, not all ~91 call sites in one pass. `LegacyApp.jsx`, `SwipeSort.jsx`, `DragToOrderTask.jsx`, `BeforeAfterImageSlider.jsx`, `TimelineCanvas.jsx`, `ExamQuestionFrame.jsx`, `ChapterCompleteScreen.jsx`, `ChapterHookScreen.jsx`, `ChapterOutcomeScreen.jsx`, `KeyFigureReveal.jsx`, `TimelineChain.jsx`, `VisualNarrativeScreen.jsx`, `ExaminerExplainsScreen.jsx`, and `VisualLearning.jsx` are done (each bar their ambiguous/foreground-colour/SVG-stroke/border/overlay sites); `BottomNav.jsx` needed no changes. Remaining files (`ModulePlayer.jsx`, `QuickFire.jsx`, `Subjects.jsx`, and others with raw `#08090D`/`#151720`/`#0D0F14`/`#0D1424`) are still unmigrated and each needs its own narrow, scoped pass — this is not a signal to broaden scope to a repo-wide migration.
