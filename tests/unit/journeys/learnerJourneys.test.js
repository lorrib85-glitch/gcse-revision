...
describe('Journey 1 — progress survives a refresh', () => {
  ...
  it('a failed write does not report success, so the app can tell progress was lost', async () => {
    ...
    expect(saveChapterState('history-medicine-black-death', { screen: 3 })).toBe(false)
  })
})
...
describe('Journey 4 — cloud restore seam', () => {
  it('richer cloud state restores onto an empty client', async () => {
    ...
    const cloud = { updatedAt: 200, data: { gcse_scores: [{ pct: 70 }], 'gcse_chapter_x': { screen: 5 } } }
  ...
  it('guest progress survives account linking — local is backed up, not wiped', async () => {
    ...
    const { saveChapterState, recordScore } = await import('../../../src/progress.js')
    ...
    saveChapterState('history-medicine-black-death', { screen: 6 })
    ...
    expect(setDocs.at(-1).data['gcse_chapter_history-medicine-black-death']).toEqual({ screen: 6 })
    ...
    await withMockedCloud(async ({ syncProgressForUser, getCloudDoc, setCloudDoc }) => {
      ...
      const cloudSnapshot = {
        ...
        data: {
          ...
          'gcse_chapter_history-medicine-renaissance-medicine': { screen: 3 },
        },
      }
      ...
      const { getChapterState, getScores } = await import('../../../src/progress.js')
      expect(getChapterState('history-medicine-black-death').screen).toBe(6)
      expect(getChapterState('history-medicine-renaissance-medicine').screen).toBe(3)
      ...
      expect(getCloudDoc().data['gcse_chapter_history-medicine-black-death']).toEqual({ screen: 6, hookDone: true })
    })
  })
})
...
describe('Journey 6 — an existing account signs in on another, older device', () => {
  it('does not undo completed learning from the newer cloud state', async () => {
    await withMockedCloud(async ({ syncProgressForUser, setCloudDoc }) => {
      ...
      saveChapterState('history-medicine-germ-theory', { screen: 3, completed: false })

      // The cloud already reflects the module being finished on another device.
      setCloudDoc({
        version: 1,
        updatedAt: Date.now(),
        data: { 'gcse_chapter_history-medicine-germ-theory': { screen: 20, completed: true, timestamp: Date.now() } },
      })

      const result = await syncProgressForUser(user)
      expect(result.action).toBe('apply')

      const resumed = getChapterState('history-medicine-germ-theory')
...
describe('Journey 7 — local and cloud each hold unique progress', () => {
  it('both valid sets survive, and repeating the sync does not duplicate anything', async () => {
    await withMockedCloud(async ({ syncProgressForUser, getCloudDoc, setCloudDoc, setDocs }) => {
      ...
      const { getChapterState } = await import('../../../src/progress.js')
      const { getWeakTopics } = await import('../../../src/unifiedWeaknessTracker.js')
      expect(getChapterState('bio_building_life').screen).toBe(5)
      expect(getChapterState('math1').screen).toBe(2)
...
describe('Journey 8 — offline learning', () => {
  it('local saves succeed while the cloud write fails, the learner keeps working, and a later retry backs up without duplicating', async () => {
    await withMockedCloud(async ({ backupProgressForUser, failNextSetDoc, getCloudDoc }) => {
      ...
      const savedOk = saveChapterState('sci_bio_w1', { screen: 4 })
      expect(savedOk).toBe(true)
      expect(getChapterState('sci_bio_w1').screen).toBe(4)
      ...
      saveChapterState('sci_bio_w1', { screen: 7 })
      ...
      expect(getCloudDoc().data['gcse_chapter_sci_bio_w1']).toEqual({ screen: 7 })
...
describe('Journey 9 — sign-out with pending progress', () => {
  it('a failed final flush does not discard local work and does not falsely report success', async () => {
    await withMockedCloud(async ({ backupProgressForUser, syncProgressForUser, failNextSetDoc, getCloudDoc }) => {
      ...
      expect(getChapterState('history-medicine-cancer')).toEqual({ screen: 9, completed: true })
...
describe('Journey 10 — auth restoration on reload', () => {
  it('existing local progress is never reset to empty while the cloud merge resolves', async () => {
    await withMockedCloud(async ({ syncProgressForUser, setCloudDoc }) => {
      ...
      saveChapterState('history-medicine-jenner-vaccination', { screen: 15, completed: true })
      const beforeSync = getChapterState('history-medicine-jenner-vaccination')
      ...
      setCloudDoc({
        version: 1,
        updatedAt: Date.now(),
        data: { 'gcse_chapter_history-medicine-great-stink': { screen: 8, completed: true } },
      })
      ...
      expect(getChapterState('history-medicine-jenner-vaccination')).toEqual(beforeSync)
      // The merged state (not a wholesale cloud overwrite) is what's live.
      expect(getChapterState('history-medicine-great-stink').completed).toBe(true)
      expect(result.action).toBe('merge')
    })
  })
})
