...
def replace_section(heading, replacement):
    global text
    pattern = rf'(?ms)^### {re.escape(heading)}\n.*?(?=^---\n|\Z)'
    text, count = re.subn(pattern, replacement.rstrip() + '\n\n', text, count=1)
    if count != 1:
        raise SystemExit(f'Expected one {heading} section, found {count}')


replace_section('QuickRecallScreen', r'''### QuickRecallScreen
...
replace_section('PriorKnowledgeRecall', r'''### PriorKnowledgeRecall
...
replace_section('MisconceptionCheck', r'''### MisconceptionCheck
...
replace_section('ChapterHookScreen', r'''### ChapterHookScreen
...
replace_section('RetrievalFrame — **LOCKED**', r'''### RetrievalFrame — **LOCKED**
...
opening_rule = "Do not stack these components simply because they are cinematic. After the opening beat, move promptly into explanation, exploration, practice or retrieval. Do not place `CinematicRevealMoment`, `ConceptReveal`, `KeyFigureReveal` and `CinematicCarousel` consecutively; that creates passive spectacle rather than learning rhythm. `CinematicRevealMoment` should be the rarest of the four because it carries the least teaching content by itself."
opening_replacement = opening_rule + " `ChapterHookScreen` is also tagged `cinematic`; when used, it fulfils the module's one cinematic-moment requirement."
if opening_rule not in text:
    raise SystemExit('Opening family rhythm rule anchor not found')
text = text.replace(opening_rule, opening_replacement, 1)
