# Canonical topic skill implementation plan

> **For agentic workers:** REQUIRED SUB-SKILL: Use superpowers:subagent-driven-development (recommended) or superpowers:executing-plans to implement this plan task-by-task. Steps use checkbox (`- [ ]`) syntax for tracking.

**Goal:** Build a project Claude Code skill, `.claude/skills/canonical-topic/SKILL.md`, invoked as `/canonical-topic <episode-or-series>`, that writes one permanent `docs/content/history/<Series>/<NN>_<Title>.md` canonical knowledge-source file per History episode — then pilot it on exactly one episode and stop for review before any further rollout.

**Architecture:** A single SKILL.md containing step-by-step instructions for an LLM to follow: resolve whether `$ARGUMENTS` names a series or a single episode, gather per-episode data from three project docs plus the conversation, resolve the output path via a directory map + filename conversion rule, and write a fixed 6-section Markdown file. Every section is written for an LLM consumer — exhaustive, bullet-structured, with explicit `MISSING:`/`UNCERTAIN:`/`CONFLICT:` flags instead of silent inference. No application code changes — this only produces planning documents.
...
**Rollout note:** This plan covers ONLY the skill build (Task 1) and a single-episode pilot (Task 2), per the spec's Rollout strategy. Task 2 ends with an explicit STOP — full Medicine Through Time series mode, and any other subject, are deliberately out of scope for this plan and become a follow-up increment once the pilot is reviewed.

---

## Task 1: Write the `canonical-topic` skill

**Files:**
- Create: `.claude/skills/canonical-topic/SKILL.md`

- [ ] **Step 1: Write the skill file**

Create `/home/user/gcse-revision/.claude/skills/canonical-topic/SKILL.md` with this exact content:
...
Run:
```bash
grep -E "^(## Step|### 2[a-f]|name:|argument-hint:)" /home/user/gcse-revision/.claude/skills/canonical-topic/SKILL.md
```
...
git commit -m "Add canonical-topic skill for History chapter knowledge sources"
git push -u origin main
```

---

## Task 2: Pilot run — Episode 1 ("Trust Me, I'm Following Jupiter")

This is the ONLY episode this plan generates output for. Per the spec's
Rollout strategy, this output is reviewed with the user before any further
episode or series is run.
...
Use the Skill tool with `skill: "canonical-topic"`, `args: "Trust Me, I'm Following Jupiter"`.

- [ ] **Step 2: Verify structure and content philosophy**

Open `docs/content/history/Medicine/01_Trust_Me_Im_Following_Jupiter.md` and
verify:

- All six `## N. <Heading>` sections present, in order, matching Step 4's
  template headings.
- Section 1 is a bulleted key:value list and states build status `Built as
  history-medicine-medieval-beliefs-causes` (the only Series 1 row
  referencing this id, per `HISTORY_SERIES_MAP.md` row 1).
- Section 1 has no "Key Topic reference" line (Series 1 has no Key Topic
  numbering).
- Section 3 ends with the 9-item Module Completion Test as an unchecked
  checklist (`- [ ] ...` × 9), matching
  `docs/system/HISTORY_MODULE_ARCHITECTURE.md`.
- Section 4 is NOT "Not yet built" — it inventories
  `history-medicine-medieval-beliefs-causes`'s existing
  `hook`/`outcomes`/`recall`/`screens`, maps each to a Section 1–6 slot, and
  ends with a `GAPS:` bulleted list.
- Section 5 is organized under the six sub-headings (Dates & timeline / Key
  people / Key terms & definitions / Case studies / Causes & effects / Exam
  angles), each as bullet lists — not paragraphs.
- If no source material for Medieval Medicine was shared earlier in this
  session, sections 2 and 5 each contain `MISSING:` bullets naming specific
  expected sub-topics (e.g. four humours, religious vs natural explanations
  of disease, treatments) — not just a bare "no material provided" sentence.
- No fact appears in the file that isn't traceable to `src/modules.js`, the
  series map, or session material — anything else is flagged `UNCERTAIN:` or
  listed under `MISSING:`.
- Overall tone/format is bullet-/structure-heavy, not prose paragraphs.

- [ ] **Step 3: Fix any issues found**

If any check in Step 2 fails because of ambiguous or incorrect SKILL.md
instructions, edit `.claude/skills/canonical-topic/SKILL.md` to clarify, then
re-run Step 1 (the skill overwrites existing output, so this is safe to
repeat) and re-check.

- [ ] **Step 4: Commit**

```bash
git add docs/content/history/Medicine/01_Trust_Me_Im_Following_Jupiter.md
git add .claude/skills/canonical-topic/SKILL.md
git commit -m "Generate pilot canonical topic file for Episode 1 (Medieval Medicine)"
git push -u origin main
```

(If Step 3 made no SKILL.md changes, the second `git add` is a no-op — that's
fine, `git commit` will just include the new file.)

- [ ] **Step 5: STOP for review**

This is the end of this plan. Present
`docs/content/history/Medicine/01_Trust_Me_Im_Following_Jupiter.md` to the
user for review. Do **not** run the skill on any further episodes or series
until the user has reviewed this pilot output and either approved it or
requested changes to `.claude/skills/canonical-topic/SKILL.md`. Per
`docs/superpowers/specs/2026-06-12-topic-brief-skill-design.md`'s Rollout
strategy: if changes are requested, refine the skill and re-run the pilot
episode; otherwise the next increment is the full Medicine Through Time
series-mode run (and, much later, other subjects).
