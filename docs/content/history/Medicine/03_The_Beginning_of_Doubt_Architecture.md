...
**Implemented runtime (Stage B, 2026-07-12):** the module is built as **12 learning screens** on the six-part spine below — `priorKnowledgeRecall` (0), `conceptReveal` (1), `quickRecall` (2), `keyFigureReveal` (3), the `theoryCompare` `people` variant (4), `quickRecall` (5), the `timelineChain` `reveal` variant (6), `factorWeb` in causes mode (7), `colsort` (8), `misconceptionCheck` (9), `examinerExplains` (10) and `guidedExamResponse` (11) — plus the module-level hook, "You will learn" outcomes and the player-owned chapter completion. The person-to-person comparison is the `theoryCompare` `people` variant, so there is no separate `SeeTheDifference` gap. Per-screen build chains, the render pass and deviations are in `03_Review_Log.md` (Stage-B entry). Implemented, not approved — a Stage-C `content-review` is still required.

Screen 6 was built as `visualNarrative` and migrated to the `timelineChain` `reveal` variant, render-verified at 390px on 2026-07-22 (`GOLD_SCREEN_REGISTER.md`). `visualNarrative` is now an unregistered screen type and its compatibility mapper has been deleted, so a rebuild must use the reveal variant — do not restore the old type from any earlier revision of this document.
