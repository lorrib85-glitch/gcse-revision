...
- **Proposed content for this episode:**
  ...
  - PriorKnowledgeRecall: concepts from Episode 9 — `rolling_thunder`, `search_and_destroy`, `body_count`, `vietcong_tactics`, `ho_chi_minh_trail`, `escalation`
...
- **Suggested component(s):**
  ...
  - `PriorKnowledgeRecall` — retrieval of Episode 9; concepts: `rolling_thunder`, `search_and_destroy`, `body_count`, `vietcong_tactics`, `ho_chi_minh_trail`

### Section 2 — Learning Chunk 1
- **Purpose:** The Tet Offensive — what happened, military result, political result.
- **Proposed content for this episode:**
  - The set-up: Westmoreland's optimism; the truce; the coordinated attack on 100+ cities
  ...
  - Political result: Cronkite's broadcast (27 February); LBJ's "if I've lost Cronkite" moment; LBJ withdraws from election (31 March 1968)
- **Suggested component(s):**
  - `VisualNarrativeScreen` — beat-by-beat: Westmoreland's "we're winning" claim → truce announced → 80,000 attack simultaneously → US Embassy breached → Hue occupied → Cronkite's "stalemate" broadcast → LBJ withdraws from election; the arc from false confidence to political collapse
  - `QuickRecallScreen` — retrieval of Rolling Thunder's failures (Episode 9) and Westmoreland's optimism (Episode 9) interleaved with Tet facts; connects the "we're winning" narrative to its collapse
...
- **Suggested component(s):**
  - `VisualNarrativeScreen` — beat-by-beat: My Lai (16 March 1968) → Thompson intervenes → report filed and buried → 14 months pass → Hersh's story (November 1969) → photographs in *Life* → outrage → Calley convicted → Nixon releases him; the moral arc of the episode's darkest content
...
- **Suggested component(s):**
  - `ExplainReveal` — cause-and-effect chain: Tet → credibility gap exposed → Cronkite editorial → LBJ withdraws → Nixon elected on "ending the war" platform → Vietnamization announced → drawdown begins (Episode 12); reveals the political consequences progressively
  - `MatchingTask` — match event to consequence: (Tet Offensive → LBJ withdraws from election), (My Lai cover-up revealed → national moral outrage), (Cronkite "stalemate" broadcast → public opinion majority against the war), (Nixon elected → Vietnamization policy); active processing of the political fallout
...
- **Proposed content for this episode:**
  ...
  - My Lai's legacy: the war crimes trial, the accountability question, the 30-year delay before Thompson was honoured
...
- **Suggested component(s):**
  - `ColSortBlock` — sort evidence into "Tet was a US/ARVN military success" vs "Tet was a US political disaster" — directly prepares students for the Q2 question about Tet as a "turning point" (where they must address both dimensions)
  - `QuickRecallScreen` — retrieval of full episode content: Tet dates, Saigon Embassy, Hue, Khe Sanh, VC casualties, Cronkite, LBJ withdrawal, My Lai (date, Calley, Thompson, cover-up, revelation), Nixon election
...
- **Proposed content for this episode:**
  ...
  - Examiner tip: the most common error is describing Tet as a VC military victory; the best answers explain WHY a military defeat for the VC was a political catastrophe for the USA (Westmoreland's false optimism, credibility gap, Cronkite, LBJ's withdrawal)
  - FaceTheExaminer: Q2-style question — "Explain why the Tet Offensive was an important turning point in the Vietnam War"
- **Suggested component(s):**
  - `ExaminerExplainsScreen` — animated Q2 structure; "Why was Tet a turning point?" as worked example; models the military vs political distinction and how to use both in a Q2 answer
