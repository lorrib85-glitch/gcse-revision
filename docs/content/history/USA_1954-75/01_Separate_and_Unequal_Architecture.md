...
- **Proposed content for this episode:**
  ...
  - PriorKnowledgeRecall: For Episode 1 (series opener), this is a prior knowledge recall of any pre-existing GCSE knowledge. Concepts to probe: Jim Crow laws, KKK, NAACP, Plessy v Ferguson, voting rights
...
- **Suggested component(s):**
  ...
  - `PriorKnowledgeRecall` — free-text recall of what learner already knows about segregation/civil rights; concepts: `jim_crow`, `kkk`, `naacp`, `plessy_v_ferguson`, `voting_rights`
...
- **Proposed content for this episode:**
  ...
  - NAACP and CORE: their roles, methods, geographic strengths
- **Suggested component(s):**
...
- **Proposed content for this episode:**
  ...
  - CORE: non-violent direct action as early philosophy; smaller but important in the North
...
- **Suggested component(s):**
  ...
  - `InteractiveCollectionExplorer` — explore "Tools of resistance" (NAACP legal strategy / CORE direct action / Women's Political Council / church networks); colour-coded by method type
...
- **Proposed content for this episode:**
  ...
  - The movement was already organised: NAACP and CORE had been fighting for decades; the 1950s movement built on existing infrastructure
...
- **Suggested component(s):**
  ...
  - `QuickRecallScreen` — retrieval of full episode content (Jim Crow scope, *Plessy*, Till, KKK, NAACP/CORE, voting denial methods)
...
- **Suggested component(s):**
  ...
  - `FaceTheExaminer` — Q2-style question: "Explain why segregation continued in the USA in the early 1950s"; marks against criteria; logs exam-technique patterns
...
2. **Emmett Till as central human throughline:** Till's story is the episode's strongest narrative moment and should be given significant screen space in Section 3. The VisualNarrativeScreen is the right component — it can deliver the full sequence (murder → open coffin → acquittal → confession → national impact) as a cinematic beat-by-beat narrative.

3. **Misconception priority:** The misconception that "the North = equality" appears in past paper mark schemes (2021 interpretations). Ensure Section 4 gives North vs South comparison enough weight, and add a MisconceptionCheck or True/False moment around "Black Americans had equality in the North."

4. **Voting rights as a distinct teaching beat:** The three barriers to voting (poll tax, literacy test, grandfather clause) are frequently examined in Q2 questions. They need their own dedicated interaction — the MatchingTask in Section 2 is the right slot.
...
6. **Component count check:** CinematicRevealMoment (S1, S5 option — keep to S1 only), VisualLearning (S2), VisualNarrativeScreen (S3), ExplainReveal (S4), ColSortBlock (S5), QuickRecallScreen (S3, S5 — both allowed under the retrieval rule exception), MatchingTask (S2). No feature component exceeds twice.
