...
- **Proposed content for this episode:**
  ...
  - Nixon's compensating strategy: bombing North Vietnam, Cambodia, Laos to buy time for Vietnamization
- **Suggested component(s):**
  - `ExplainReveal` — cause-and-effect chain: domestic anti-war pressure (Episode 11) + Congress's Gulf of Tonkin repeal → Nixon needs an exit → Nixon Doctrine/Vietnamization → ARVN must be able to fight independently → training and equipment → but ARVN's fundamental problems remain; reveals the gap between the strategy's logic and its premises
  - `QuickRecallScreen` — retrieval of ARVN failures (Episodes 8–9: Ap Bac, Search and Destroy, Tet) interleaved with Vietnamization facts; connects the history of ARVN ineffectiveness to why Vietnamization was structurally unlikely to work
...
- **Proposed content for this episode:**
  ...
  - Easter Offensive (April 1972): North Vietnam launched a major NVA conventional attack; ARVN performed poorly; Nixon responded with Operation Linebacker (intensive bombing); held the line through air power alone — not ARVN performance
...
- **Suggested component(s):**
  - `VisualNarrativeScreen` — beat-by-beat: Operation Lam Son 719 (ARVN invades Laos → rapid retreat → soldiers on helicopter skids → Nixon downplays it) → Easter Offensive (NVA attacks → ARVN fails → Linebacker saves the day via bombing) → the conclusion: two years of Vietnamization and the ARVN still can't fight alone
  - `QuickRecallScreen` — retrieval of Ap Bac (Episode 8) and Gulf of Tonkin Resolution repeal (Episode 11) interleaved with Lam Son 719/Easter Offensive facts; the series-long ARVN failure thread completing
...
- **Proposed content for this episode:**
  ...
  - Watergate and its consequences: Nixon resigned August 1974; Ford could not secure Congressional funding; War Powers Act (November 1973) constrained future presidents
- **Suggested component(s):**
  ...
  - `MatchingTask` — match cause to consequence: (NVA troops left in South → could resume offensive immediately after US withdrawal), (Watergate → Nixon resigned, promise to Thieu worthless), (Congress cut off funds → Ford powerless to support South Vietnam), (War Powers Act → future presidents constrained); active processing of the domino of political failures
