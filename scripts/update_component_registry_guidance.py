...
text = path.read_text()
text = re.sub(r'\*\*Last updated:\*\* \d{4}-\d{2}-\d{2}', '**Last updated:** 2026-07-26', text, count=1)

builder = r'''### BuilderBlock
...
if '### BuilderBlock\n' not in text:
    text = text.replace('### CalculationBreakdown\n', builder + '### CalculationBreakdown\n', 1)

calculation = r'''### CalculationBreakdown
...
text, count = re.subn(r'### CalculationBreakdown\n.*?\n---\n', calculation, text, count=1, flags=re.S)
if count != 1:
    raise SystemExit('CalculationBreakdown section was not replaced exactly once')

fill = r'''### FillInTheBlanksBlock
...
text, count = re.subn(r'### FillInTheBlanksBlock\n.*?\n---\n', fill, text, count=1, flags=re.S)
if count != 1:
    raise SystemExit('FillInTheBlanksBlock section was not replaced exactly once')
