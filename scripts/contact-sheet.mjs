#!/usr/bin/env node
// Chapter contact sheet — renders every screen of a chapter in the real render
...
//
// Usage:
//   node scripts/contact-sheet.mjs <chapter-id> [--full]
//
...
//              Must be a dev server — the ?chapter=<id>&screen=<n> jump seam
//              in LegacyApp.jsx is import.meta.env.DEV only.
//
// Output: contact-sheets/<chapter-id>/ (gitignored), overwritten per run.
...
// settle margin for lazy chapter content and reduced-motion resting states.
...
if (!chapter || !chapter.screenCount) {
  const available = CHAPTERS.filter(m => m.screenCount > 0).map(m => `  ${m.id} (${m.screenCount} screens)`)
  console.error(chapter ? `Chapter "${chapterId}" has no screens.` : `Unknown chapter id: ${chapterId ?? '(none given)'}`)
  console.error(`\nAvailable chapters:\n${available.join('\n')}`)
  process.exit(2)
}

const outDir = path.join(REPO_ROOT, 'contact-sheets', chapter.id)
...
console.log(`\n${chapter.id} — "${chapter.title}" — ${chapter.screenCount} screens\n`)

const frames = []

await capture(`/?chapter=${encodeURIComponent(chapter.id)}`, 'frame-000-hook.png')
frames.push({ file: 'frame-000-hook.png', label: 'hook' })

// The jump seam only marks hook/wyl/intro done for screen > 0, so seed the
// chapter state ourselves before asking for screen 0 (guest scope, see
...
for (let i = 0; i < chapter.screenCount; i++) {
  const tag = chapter.screenTags?.[i] || null
  const file = `frame-${String(i + 1).padStart(3, '0')}-s${String(i).padStart(2, '0')}${tag ? `-${tag}` : ''}.png`
  await capture(`/?chapter=${encodeURIComponent(chapter.id)}&screen=${i}`, file)
...
<meta charset="utf-8">
<title>${esc(chapter.title)} — contact sheet</title>
<style>
...
<p class="meta">${esc(chapter.id)} · ${chapter.screenCount} screens + hook · ${fullPage ? 'full-page' : '390×844 fold'} · generated ${new Date().toISOString().slice(0, 16).replace('T', ' ')}</p>
<div class="grid">
