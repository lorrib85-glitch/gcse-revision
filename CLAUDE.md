...
| `src/data/masteryEngine/` | **Canonical learner mastery engine** — pure logic layer recording what one learner knows, as per-concept evidence keyed by registered learning graph concept ids (unknown ids throw). Mastery/confidence/strength are derived at read time, never stored; persistence only via its `masteryStore.js` through `src/lib/storage.js`. App/UI consumers are intentionally blocked (guard test in `tests/architecture/mastery-engine.test.js`) until a dedicated consumer phase is explicitly authorised. See `docs/system/MASTERY_ENGINE.md`. |
